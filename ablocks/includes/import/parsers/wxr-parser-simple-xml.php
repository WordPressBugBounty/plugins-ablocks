...
class WXR_Parser_SimpleXML {
	public function parse( $file ) {
		...
		$show_on_front = $xml->xpath( '/rss/channel/wp:options/show_on_front' );
		if ( $show_on_front ) {
			$show_on_front = (string) trim( $show_on_front[0] );
		}

		$page_on_front = $xml->xpath( '/rss/channel/wp:options/page_on_front' );
		if ( $page_on_front ) {
			$page_on_front = (int) $page_on_front[0];
		}

		$page_for_posts = $xml->xpath( '/rss/channel/wp:options/page_for_posts' );
		if ( $page_for_posts ) {
