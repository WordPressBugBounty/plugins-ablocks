...
class CssGeneratorV2 {
	...
	public function add_class_styles( $class_name, $desktop_styles, $tablet_styles = [], $mobile_styles = [], $responsive_styles = [] ) {
		$this->class_styles[] = [
			...
			// Optional map of custom breakpoints: [ suffix => [ width, styles ] ].
			'responsive_styles' => $responsive_styles,
		];
	}

	/**
	 * Build the custom-breakpoint style map for a selector from a per-device
	 * builder closure. Only user-registered custom breakpoints (not the built-in
	 * Tablet/Mobile) are included, so the standard path is untouched.
	 */
	public function custom_device_map( $builder ) {
		...
		// Phantom baseline: the builder run against a suffix that has no stored
		// values. Controls that hardcode only Tablet/Mobile defaults emit
		// "phantom" declarations (e.g. border-width:0) for any unknown suffix;
		// subtracting the phantom leaves only genuinely-overridden custom values,
		// with no per-control changes.
	...
	public function generate_css() {
		...
		foreach ( $this->class_styles as $class_style ) {
			...
			if ( empty( $class_style['responsive_styles'] ) ) {
				...
				// Custom breakpoints present: emit every breakpoint in
				// width-descending order (narrowest last so it wins the cascade),
				// each deduped against what it inherits.
